...
def simulate_activate_func():
    ...
    act_func = input("Mời bạn nhận tên activation function chỉ có 3 loại (sigmoid, relu, elu) ")
...
def calc_regres_loss():
    ...
    if not(number_samples.isnumeric()):
        ...
        for _ in range(int(number_samples)):
            predict.append(np.random.uniform(0, 10)) 
            target.append(np.random.uniform(0, 10)) 
        mae = 0.0
        mse = 0.0
        rmse = 0.0
